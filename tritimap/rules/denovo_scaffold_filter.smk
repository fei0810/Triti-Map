...
rule uniqScaffold:
    ...
    shell:"""
    set +e
    bash {params.scriptdir}/getuniqscaffold.sh {input.fa} {input.region} {params.length} {params.genome} {threads} {params.datatype} {params.memory} {output.scaffold1} {output.scaffold2} {output.unmap1} {output.unmap2} {output.scaffold1summary} {output.scaffold2summary}  > {log} 2>&1
    exitcode=$?
    if [ $exitcode -eq 1 ]
    then
        exit 1

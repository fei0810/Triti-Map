...
rule uniqScaffoldByRegion:
    ...
    params:
        ...
        identicalfilter = config['bulk_specific']['identical_percentage'],
        matchlengthfilter = config['bulk_specific']['length_percentage']
    output:
    ...
    bash {params.scriptdir}/getuniqscaffold.sh {input.fa} {output.region} {params.length} {params.genome} {threads} {params.datatype} {params.memory} {output.scaffold1} {output.scaffold2} {output.unmap1} {output.unmap2} {output.scaffold1summary} {output.scaffold2summary} {params.identicalfilter} {params.matchlengthfilter} > {log} 2>&1
    exitcode=$?
    if [ $exitcode -eq 1 ]
    then
        exit 1
